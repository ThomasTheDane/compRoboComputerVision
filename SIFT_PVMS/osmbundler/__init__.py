...
class OsmBundler():
    ...
    def checkCamerasInDatabase(self):
        logging.info("checkCamerasInDatabase")
        # open connection to cameras database
        conn = sqlite3.connect(camerasDatabase)
        self.dbCursor = conn.cursor()
    ...
    def checkCameraInDatabase(self, photoPath):
        logging.info("!!! checkCameraInDatabase")
        ...
        if 'Make' in exif and 'Model' in exif:
            ...
            logging.info("pass 4")
            # logging.info(exifMake)
            # logging.info(exifModel)
            ccdWidth = self.getCcdWidthFromDatabase(exifMake, exifModel)
            # logging.info(ccdWidth)
            if ccdWidth==None:
                while True:
                ...
                logging.info("Camera is already inserted into the database")
                print "camera in database"
                return
    ...
    def _calculateFocalDistance(self, photo, photoInfo, exif):
        # logging.info("_calculateFocalDistance")
        hasFocal = False
        ...
        if focalLength>0 and width>0 and height>0:
            ...
            focalPixels = width * (focalLength / ccdWidth)
            hasFocal = True
            self.bundlerListFile.write("%s.jpg 0 %s\n" % (photo,SCALE*focalPixels))

        # if 'Make' in exif and 'Model' in exif:
        #     # logging.info("     Make:", exif['Make'].strip())
        #     # logging.info("     Model:", exif['Model'].strip())
        #     # check if we have camera entry in the database
        #     ccdWidth = self.getCcdWidthFromDatabase(exif['Make'].strip(),exif['Model'].strip())
        #     logging.info("      old ccdWidth: " + str(ccdWidth))
        #     ccdWidth = 3.76
        #     logging.info("      new ccdWidth: " + str(ccdWidth))
        #     if ccdWidth:
        ...
        #             logging.info("      old focalLength: " + str(focalLength))
        #             logging.info("      old width: " + str(width))
        #             logging.info("      old height: " + str(height))
        ...
        # else:
        #     # logging.info("\tCamera not in database. Calculating focal length manually.")p
        #     pass
        #     # focal pixel = (focal mm / sensor width in mm) * image width in pixels
        #     # samsung galaxy s5: focal length 35mm, sensor size: 1/2.5" or 1/2.6"
        if not hasFocal:
